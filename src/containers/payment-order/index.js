...
import { billActions, paymentOrderActions } from "../../actions";
...
const listPaymentMethodsRefund = [6, 10, 7, 14];
class TransPaymentOrder extends Component {
  constructor(props) {
    super(props);
    this.state = {
      searchBody: {},
    ...
    if (amountRefundState > 0 && selectList.length > 0) {
      let modal = Modal.confirm({
        ...
        content: (
          <div>
            <label>
              {translate("ARE_YOU_SURE_YOU_REFUND_AMOUNT", {
                money: globalProps.inputNumberVND.formatter(amountRefundState),
              })}
            </label>
          </div>
        ),
      ...
      let modal = Modal.warning({
        ...
        content: (
          <div>
            <label>{translate("INVALID_AMOUNT_REFUND")}</label>
          </div>
        ),
        cancelText: translate("CLOSE"),
        onCancel: () => {
  ...
  onRowChange(index, key, value) {
    ...
    if (item) {
      ...
      item[key] = value;
      if (key === "reasonRefund") {
        item.reasonRefundName = listResourceSetting.filter(
          (e) => e.code === value
        )[0].name;
      }
  ...
  render() {
    ...
    return (
      <React.Fragment>
          ...
          >
            <Row {...globalProps.row}>
              ...
              <Col {...globalProps.col}>
                <Form.Item
                  {...globalProps.formItem}
                  label={<Translate id="LABEL_CHART_WALLET" />}
                  name="paymentType"
                >
                  <Select {...globalProps.selectSearch} allowClear>
            ...
            >
                ...
                render={(text, record) => (
                  <Row gutter={8} style={{ flexWrap: "nowrap" }}>
                    {isAllow(PERMISSION.PAYMENT_TRANSACTION.PROCESS) && (
                      <Space>
                        {listPaymentMethodsRefund.includes(
                          record.paymentType 
                        ) &&
                        ...
                        record.amountRefund === 0 ? (
                          <Col flex="32px">
                            <Button
                              ...
                              className="custom-btn-primary"
                              onClick={() =>
                                this.showTopupModal(record, "REFUND")
                              }
                            >
                        ...
                        record.amountRefund > 0 ? (
                          <Col flex="32px">
                            ...
                            <Button
                              ...
                              className="custom-btn-primary"
                              onClick={() =>
                                this.showTopupModal(record, "RE_REFUND")
                              }
                            >
              ...
              <Column
                ...
                render={(k, i) => {
                  return paymentMethods &&
                    ...
                    k
                    ? paymentMethods.find((e) => e.id === k)
                      ? paymentMethods.find((e) => e.id === k).name
                      : k
                    : "";
                }}
              />
              ...
              <Column
                ...
                render={(val, record) => (
                  <RenderText
                    value={
                      record.orderAmount -
                      record.orderPromoAmount -
                      record.orderPartnerPromoAmount
                    }
                    type="NUMBER_NO_DOT"
                  />
                )}
              ...
              <Column
                ...
                render={(val, record) => (
                  <RenderText
                    value={
                      record.orderPromoAmount + record.orderPartnerPromoAmount
                    }
                    type="NUMBER_NO_DOT"
                  />
                )}
              ...
              <Column
                ...
                render={(val) =>
                  val ? (
                    <Translate id={`TRANSACTION_REFUND_STATUS.${val}`} />
                  ) : (
                    ""
                  )
                }
        ...
        {/* Topup  */}
        <Modal
          title={<strong>Xử lý giao dịch</strong>}
          textAlign="center"
          centered={true}
              ...
              <Row {...globalProps.row}>
                <Col {...globalProps.col3}>
                  <Form.Item label={"Mã đơn hàng"}>
                    <Input
                      disabled={true}
                      value={itemRefund[0] && itemRefund[0].billNumber}
                    />
                  </Form.Item>
                  <Form.Item label={"Số tiền hoàn"}>
                    <strong style={{ fontSize: 20, color: "#FF6666" }}>
                      {globalProps.inputNumberVND.formatter(amountRefundState)}
                    </strong>
                  </Form.Item>
                </Col>
              </Row>
                        ...
                        rules={[
                          ({ getFieldValue }) => ({
                            validator(_, value) {
                              let isValid = false;
                              if (
                                selectIndexList.length > 0 &&
                                selectIndexList.includes(record.key)
                              ) {
                                if (value != null) {
                                  isValid = true;
                                } else {
                              ...
                              return Promise.reject(new Error("Yêu cầu chọn!"));
                            },
                          }),
                        ]}
                      >
                        ...
                        rules={[
                          ({ getFieldValue }) => ({
                            validator(_, value) {
                              ...
                              return Promise.reject(new Error("Yêu cầu điền!"));
                            },
                          }),
                        ]}
                      >
...
const stateToProps = (state) => {
  return {
    machines: state.paymentOrder.machines,
    paymentMethods: state.paymentOrder.paymentMethods,
    locations: state.paymentOrder.locations,
    paymentStatusList: state.paymentOrder.paymentStatusList,
    fromDate: state.paymentOrder.fromDate,
    toDate: state.paymentOrder.toDate,
    fromTime: state.paymentOrder.fromTime,
    toTime: state.paymentOrder.toTime,
    paymentTransactionData: state.paymentOrder.paymentTransactionData,
    total: state.paymentOrder.total,
    listOrderDetail: state.bill.listOrderDetail,
    totalOrderDetail: state.bill.total,
    listResourceSetting: state.bill.listResourceSetting,
    totalAmount: state.paymentOrder.totalAmount,
    totalRefund: state.paymentOrder.totalRefund,
  };
};
...
const dispatchToProps = (dispatch) =>
  bindActionCreators(
    {
      init: paymentOrderActions.init,
      search: paymentOrderActions.search,
      exportExcel: paymentOrderActions.exportExcel,
      getListOrderDetail: billActions.getListOrderDetail,
      refund: paymentOrderActions.refund,
      reRefund: paymentOrderActions.reRefund,
    },
    dispatch
  );
...
export default connect(
  stateToProps,
  dispatchToProps
)(withLocalize(TransPaymentOrder));
